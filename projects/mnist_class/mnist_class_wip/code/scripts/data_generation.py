...
def run_test_batch(start_batch, wsd_folder):
    network = Network()
    ...
    print(os.path.join(config_dir, "network", wsd_folder))
    ...
    create_network(
        ...
        ile_synapse_specs,
        storage="load",
        storage_path=os.path.join(config_dir, "network", wsd_folder),
    )
    synapse_specs_list = [
        efe_synapse_specs,
        ele_synapse_specs,
    ...
    final_layer_monitor = SpikeMonitor(exc_neuron_specs.neuron_groups[4], record=True)
    stimulus_time = 100*ms
    network.run(stimulus_time)
    ...
    for batch in range(NO_BATCHES):
        ...
        for item_idx in range(batch_size):
            ...
            device.run(run_args={input_layer.rates: input_rates})
            print("Network run complete for batch", batch, "item", item_idx)
...
for epoch in range(3):
    for start in range (0, 60000, 500):
        ...
        wsd_folder = f"epoch_{epoch}_item_{start}"
        end_folder = f"epoch_{epoch}_item_{start + 500}"
        print(f"Running training from items {start} to {start+500} (batches {start_batch} to {start_batch+10})")
        run_training_batch(start_batch, wsd_folder, end_folder)

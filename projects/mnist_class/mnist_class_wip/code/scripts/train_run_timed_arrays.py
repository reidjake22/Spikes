...
def run_training_batch(start_batch, start_folder, end_folder ):
    ...
    input_timed_array = np.zeros((NO_BATCHES*batch_size, 4096))  # Assuming each image is 64x64 pixels flattened to 4096
    ...
    for batch in range(NO_BATCHES):
        ...
        input_timed_array[batch*batch_size:(batch+1)*batch_size, :] = neuron_inputs
    input_timed_array = input_timed_array * 6000 * Hz
    ...
    device.build(clean=True, run=False)
    device.run()
...
for epoch in range(1):
    for start in range (0, 500, 500):
        ...
        if start == 0 and not epoch == 0:
            retrieval_epoch = epoch -1
            start = 60000
        start_folder = f"epoch_{retrieval_epoch}_item_{start}"
        end_folder = f"epoch_{retrieval_epoch}_item_{start + 500}"

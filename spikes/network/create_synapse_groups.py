...
def create_synapse_groups(
    ...
    logger.info(f"creating {n_layers} synapse layers")
    for layer in range(1, n_layers + 1):
        logging.debug(f"creating synapses for layer {layer}")
        # Create efe synapses for all layers except the last
        if not layer == n_layers:
            # create efe synapses
            logging.debug(f"\r*** creating efe synapses for layer {layer} ***")
            ...
            logging.debug(f"\r*** connecting efe synapses for layer {layer} ***")
        ...
        # create ele synapses
        logging.debug(f"\r*** creating ele synapses for layer {layer} *** ")
        ...
        if not layer == 1:
            # create ebe synapses
            logging.debug(f"\r*** creating ebe synapses for layer {layer}")
        ...
        # create E-I synapses
        logging.debug(f"\r*** creating eli synapses for layer {layer} *** ")
        ...
        # create I-E synapses
        logging.debug(f"\r*** creating ile synapses for layer {layer} *** ")

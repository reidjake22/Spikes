...
class NeuronParameters:
    ...
    def check_valid_parameters(self, neuron_type):
        ...
        if neuron_type in params_by_type:
            ...
            missing_required = [p for p in required if getattr(self, p) is None]
            forbidden_present = [p for p in forbidden if getattr(self, p) is not None]
            if missing_required or forbidden_present:
                logger.warning(
                    f"Neuron type '{neuron_type}' has inconsistent parameters. "
...
class NeuronSpecs:
    ...
    def add_neurons(self, layer, neuron_group):
        ...
        if self.neuron_groups.get(layer):
            logger.warning(f"Neuron group for layer {layer} already exists.")
        else:
    ...
    def create_neurons(self, layer, target_network=None):
        ...
        if not target_network == None:
            logger.debug(f"adding layer{layer} neurons to network")

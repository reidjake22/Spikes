        ...
        if self.type == "f":
            ...
            for key in [
                ...
                if not hasattr(self, key) or getattr(self, key) is None:
                    missing.append(key)
            if not missing == []:
                logger.debug(f"Missing parameters unless eli or ile: {missing}")
...
class SynapseSpecs:
    ...
    def connect_synapses(
        ...
        logger.debug(f"\r*** Connecting {afferent_group.name}→{efferent_group.name} (layer {layer}) \n radius={radius}, scale={scale} ***")

        # 2) try loading
        if storage == "load":
            try:
                logger.debug(storage_path)
                data = np.load(os.path.join(
                    storage_path,
                    f"{layer}_{afferent_group.name}_{efferent_group.name}_synapses.npz"
                ))
                logger.debug(data)
                logger.debug("Loading synapses from file")
                i=data["i"]
                j=data["j"]
                logger.debug("connecting synapses")
                synapses.connect(i=i, j=j)
                logger.debug("setting weights and delays")
                w = data["w"]
                logger.debug(f"weights {w}")
                synapses.w = w
                delay = data["delay"]
                logger.debug(f"delays:{delay}")
                synapses.delay = delay * msecond
                logger.debug("setting synapse parameters")
                self._set_synapse_parameters(synapses)
                if self.recent_a == self.recent_e:
                    synapses.plasticity = 1
                return
            except Exception as e:
                logger.warning(f"Error loading synapses: {e}")
                logger.warning("Load failed, regenerating…")
                ...
                if response.lower() == 'yes':
                    storage = "save"
        logger.info("Generating synapses from scratch")
        # 3) precompute the offsets & centers once
        logger.debug("Precomputing offsets and centers")
        ...
        logger.debug("gathered rows and cols")
        ...
        logger.debug("applying clippings")
        ...
        logger.debug("probabilistically culling")
        # 5) decide which to keep
        max_conn = counts.max()
        logger.debug(f"max_conn {max_conn}")
        p = avg_no_neurons / max_conn
        logger.debug(f"p {p}")
        keep = np.random.rand(i_inds.size) < p
        logger.debug(f"keeping {keep.sum()} connections")
        logger.debug (f"i_inds {i_inds[keep]}")
        logger.debug(f"j_inds {j_inds[keep]}")
        # 6) one‐shot connect
        if keep.any():
            synapses.connect(i=i_inds[keep], j=j_inds[keep])
        else:
            logger.warning("Warning: no connections kept!")

        # 7) set your STDP/non‐STDP parameters and save if needed
        self._set_synapse_parameters(synapses)

        # 8) set weights and delays
        if self.recent_a == self.recent_e:
            synapses.w = "rand()"
        ...
        if storage == "save":
            ...
            logger.info("Synapses saved to file")
    ...
    def _set_synapse_parameters(self, synapses):
        ...
        for param in safe_values:
            try:
                ...
                pass
        logger.debug(f"*** Set values: {set_values} ***")
        logger.debug(f"*** Excluded values: {excluded_values} ***")

...
if __name__ == "__main__":
    ...
    run_training(
        network, namespace, STIMULUS_LENGTH, NUM_INPUTS, no_epochs=NO_EPOCHS + 1
    )
    ...
    # Doesn't work:
    # plot_synapse_distributions(efe_synapse_specs)
    # plot_synapse_distributions(ele_synapse_specs)
    # plot_synapse_distributions(ebe_synapse_specs)
    for key, item in monitors.items():
        plt.figure()
        plt.hist(
            item.t / ms,
            bins=int(max(item.t / ms)),
            cumulative=True,
            histtype="step",
            label=f"Layer {key}",
        )
    ...
    network_monitors.animate_spike_heatmap(
        1, "spike", NUM_INPUTS, TEST_STIMULUS_LENGTH, exc_neuron_specs.length
    )
    network_monitors.animate_spike_heatmap(
        2, "spike", NUM_INPUTS, TEST_STIMULUS_LENGTH, exc_neuron_specs.length
    )

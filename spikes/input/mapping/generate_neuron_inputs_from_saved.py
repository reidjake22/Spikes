...
def generate_neuron_inputs_from_array(
        convolved: np.ndarray,
        mapping_path: str,
        neuron_input_path: str
):
    m = np.load(mapping_path)
    indices = m['indices']
    indptr = m['indptr']
    ...
    for j in range(N):
        start, end = indptr[j], indptr[j+1]
        vals = flat[:, indices[start:end]]
        # zero out negatives then sum
        inputs[:, j] = np.clip(vals, a_min=0, a_max=None).sum(axis=1)
    # save to file
    np.save(neuron_input_path, inputs)
    print(f"Neuron inputs saved to {neuron_input_path}")

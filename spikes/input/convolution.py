...
def convolve_image_with_gabor_filters(image, gabor_filters):
    ...
    for filter_idx, gabor_filter in enumerate(gabor_filters.filters):
        convolved_image[:, :, filter_idx] = convolve2d(image, gabor_filter.filter_array, mode='same', boundary='wrap')
...
def convolve_dataset_with_gabor_filters(dataset, gabor_filters):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        futures = {executor.submit(convolve_image_with_gabor_filters, image, gabor_filters): idx for idx, image in enumerate(dataset)}
        
        # Use tqdm to display a progress bar and process results as they are completed
        for future in tqdm(as_completed(futures), total=num_images, desc="Convolving images"):
